 ...
 * @author huajie <banhuajie@163.com>
 */
function action_log($action = null, $model = null, $record_id = null, $user_id = null, $info = ''){
    ...
    if(!empty($action_info['log'])){
        ...
        //未定义日志规则，记录操作url
        $data['remark']     =   '操作url：'.$_SERVER['REQUEST_URI'] . ':' . $info;
    }
...
function action_user_log($info) {
    action_log('user_action', 'document', 1, UID, $info);
}
/**
 * 解析行为规则
...
function echo_json_die($obj) {
    echo json_encode($obj);die();
}


function result($success = TRUE, $message = '', $data = NULL, $content_type = 'obj', $callback = '', $error_code = '') {
    $result = new \stdClass();
...
function result_json($success = TRUE, $message = '', $data = NULL, $error_code = 100) {
    $callback = I('get.callback');//因为是方法,所以只能这么处理了,之后会把返回设计成一个类
    if ($success) {
        $error_code = 0;
    }
    echo result($success, $message, $data, 'json', $callback, $error_code);
    exit;
}
...
function convert_objs($obj_arr, $field_arr, $split_word = ',', $change_type = FALSE, $up_field = '') {
    ...
    foreach ($obj_arr as $obj) {
        $convert_obj = convert_obj($obj, $field_arr, $split_word, $change_type, $up_field);
        if ($convert_obj && is_object($convert_obj)) {
            $new_obj_arr[] = $convert_obj;
        }
    }
    return $new_obj_arr;
}

function convert_obj($obj, $field_arr, $split_word = ',', $change_type = FALSE, $up_field = '') {
    if (!$field_arr) {
        return $obj;
    }
    if (!is_array($field_arr)) {
        if (is_string($field_arr)) {
            $field_arr = str_replace(' ', '', $field_arr);
            $field_arr = explode($split_word, $field_arr);
        } else {
            return $obj;
        }
    }
    ...
    foreach ($field_arr as $field) {
        ...
        $field = isset($field_explode[0]) ? $field_explode[0] : '';
        $new_field = isset($field_explode[1]) ? $field_explode[1] : '';
        $new_field = $new_field ? $new_field : $field;
        if (is_object($obj)) {
            if (isset($obj->$field)) {
                if ($change_type) {
                    $new_obj->$new_field = change_type($obj->$field);
                } else {
                    $new_obj->$new_field = $obj->$field;
                }
            }
    ...
    if ($up_field) {
        $up_field_arr = array();
        if (!is_array($up_field)) {
            if (is_string($up_field)) {
                $up_field_arr = explode($split_word, $up_field);
            }
        }
        if ($up_field_arr) {
            foreach ($up_field_arr as $field) {
                if (isset($new_obj->$field)) {
                    if (is_object($new_obj->$field)) {
                        foreach ($new_obj->$field as $_key => $_value) {
                            if ($change_type) {
                                $new_obj->$_key = change_type($_value);
                            } else {
                                $new_obj->$_key = $_value;
                            }
                        }
                        unset($new_obj->$field);
                    } else if (is_array($new_obj->$field)) {
                        foreach ($new_obj->$field as $_key => $_value) {
                            if ($change_type) {
                                $new_obj->$_key = change_type($_value);
                            } else {
...
function has_more($count, $p, $page_size) {
    if ($count > $p * $page_size) {
        return TRUE;
    }
    return FALSE;
}
...
function tel_num_security($tel) {
    $tel_1 = substr($tel, 0, 3);
    $tel_2 = substr($tel, -4, 4);
    return $tel_1. '***' . $tel_2;
}
