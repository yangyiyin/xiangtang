...
class NeedsTypesService extends BaseService{
    public static $name = 'NeedsType';
    ...
    public function get_all_types_option($selected_id = '', $where=[]) {
        $NfModel = D('Nf' . static::$name);
        $where['deleted'] = ['EQ', static::$NOT_DELETED];
        ...
        if ($all) {
            foreach ($all as $_item) {
                if ($selected_id && $selected_id == $_item['id']) {
                    $options .= '<option selected="selected" value="'.$_item['id'].'">'.$_item['title'].'</option>';
                } else {
                    $options .= '<option value="'.$_item['id'].'">'.$_item['title'].'</option>';
                }
            }
        }
        return $options;
    }
    public function get_normal_types() {
        $NfModel = D('Nf' . static::$name);
        $where = [];
        $where['is_normal'] = ['eq', 1];
        $where['deleted'] = ['EQ', static::$NOT_DELETED];
        return $NfModel->where($where)->select();
    }

    public function get_unnormal_types() {
        $NfModel = D('Nf' . static::$name);
        $where = [];
        $where['is_normal'] = ['eq', 0];
        $where['deleted'] = ['EQ', static::$NOT_DELETED];
        return $NfModel->where($where)->select();
    }
