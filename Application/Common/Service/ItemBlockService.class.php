 ...
 * Time: 2017-08-11 14:40:03
 */
namespace Common\Service;
class ItemBlockService extends BaseService{
    public static $name = 'ItemBlock';
    ...
    public function get_by_iids_type($iids, $type){
        $NfModel = D('Nf' . static::$name);
        $where = [];
        $where['iid'] = ['in', $iids];
        $where['type'] = ['eq', $type];
    ...
    public function set_block($iids, $type){
        ...
        if ($items) {
            $exists = result_to_array($items, 'iid');
            $news = array_diff($iids, $exists);
            if ($news) {
                $data = [];
                foreach ($news as $iid) {
                    $data[] = ['iid'=>$iid, 'type'=>$type];
                }
