...
class ArticleClicksService extends BaseService{
    public static $name = 'ArticleClicks';
    ...
    public function get_info_by_aid($aid,$type) {
        $NfModel = D('Nf' . static::$name);
        $where = [];
        $where['aid'] = ['EQ', $aid];
        $where['type'] = ['EQ', $type];
    ...
    public function get_by_aids($aids) {
        if (!check_num_ids($aids)) {
            return [];
        }
        $NfModel = D('Nf' . static::$name);
        $where = [];
