 ...
 * Time: 2017-07-23 12:23:41
 */
namespace Common\Service;
class UserInviterCodeService extends BaseService{
    public static $name = 'UserInviterCode';
    ...
    public function del_by_uids($uids) {
        if (!check_num_ids($uids)) {
            return false;
        }
        $NfModel = D('Nf' . static::$name);
        $where = ['uid' => ['in', $uids]];
        $ret = $NfModel->where($where)->delete();
        if ($ret) {
            return result(TRUE);
        } else {
            return result(FALSE, '网络繁忙~');
