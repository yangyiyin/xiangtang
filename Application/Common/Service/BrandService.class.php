...
class BrandService extends BaseService{
    ...
    public function get_all_options($selected_id) {
        $NfModel = D('Nf' . static::$name);
        $where = [];
        $where['deleted'] = ['EQ', static::$NOT_DELETED];
        ...
        if ($all) {
            foreach ($all as $_item) {
                if ($selected_id && $selected_id == $_item['id']) {
                    $options .= '<option selected="selected" value="'.$_item['id'].'">'.$_item['name'].'</option>';
                } else {
                    $options .= '<option value="'.$_item['id'].'">'.$_item['name'].'</option>';
                }
            }
        }
        return $options;
